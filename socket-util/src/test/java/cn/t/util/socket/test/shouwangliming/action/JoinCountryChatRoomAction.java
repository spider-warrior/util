...
public class JoinCountryChatRoomAction {
    public void sendMsg(WebSocketClient socketClient) {
        Map<String, Object> countryParams = new HashMap<>();
        countryParams.put("group", "country");
        countryParams.put("roomId", "country_481");
        try {
            socketClient.send(JsonUtil.serialize(new WebSocketMsg(Cmd.ROOM_JOIN, countryParams)));
